...
def tracked_workshop_files(repo_root: Path) -> list[Path]:
    try:
        ...
        git_root = Path(top_level_process.stdout.strip()).resolve()
...
def should_include(path: Path) -> bool:
    ...
    if any(part in excluded_parts for part in path.parts):
        return False
    if path == Path(".env") or path.suffix.lower() in {".pyc", ".pptx", ".pdf", ".docx"}:
        return False
...
def create_workspaces(
    ...
    try:
        for participant_id, destination in zip(participant_ids, destinations):
            ...
            environment_example_text = render_environment(
                template_text,
                assignments,
            )
